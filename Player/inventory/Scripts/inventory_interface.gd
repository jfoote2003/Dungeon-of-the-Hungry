...
func set_member1_inventory_data(member : Combatant):
	connect_to_equipment_slots(member)
	#the physical slot
	game_menu.set_player1_inventory_data(member)

func set_member2_inventory_data(member : Combatant):
	connect_to_equipment_slots(member)
	#the physical slot
	game_menu.set_player2_inventory_data(member)

func set_member3_inventory_data(member : Combatant):
	connect_to_equipment_slots(member)
	#the physical slot
	game_menu.set_player3_inventory_data(member)

func set_member4_inventory_data(member : Combatant):
	connect_to_equipment_slots(member)
	#the physical slot
...
func connect_to_equipment_slots(member : Combatant):
	...
	member.ring1_inv_data.inventory_interact.connect(on_inventory_interact)
	member.ring2_inv_data.inventory_interact.connect(on_inventory_interact)
